...
export MASTER_PORT=7066
export PYTHONPATH=$PYTHONPATH:.../OFA/fairseq

log_dir=./twitter10000_large_logs
save_dir=./twitter10000_large_checkpoints
...
for max_epoch in 6; do
  ...
  for lr in 2e-5; do
    ...
    CUDA_VISIBLE_DEVICES=0 python3 -m torch.distributed.launch --nproc_per_node=1 --use_env --master_port=${MASTER_PORT} ../../train.py \
        $data \
        ...
        --weight-decay=0.2 --optimizer=adam --adam-betas="(0.9,0.999)" --adam-eps=1e-08 --clip-norm=1.0 \
        ...
        --save-interval-updates=50000 --validate-interval-updates=50000 \
